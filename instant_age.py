...
def main(args):
    ...
    img_crop_2 = cv2.imread('./examples/514.jpg')
    ...
    for i, frame in tqdm(enumerate(reader)):
        ...
        if len(queue_ver) >= n:
            ...
            # out.write(all)
            all = cv2.resize(all, (1960,1080))
            cv2.namedWindow("Full-Pose", cv2.WINDOW_AUTOSIZE)
            cv2.resizeWindow("Full-Pose",1960,1080)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--resume_iter', type=int, default=803000,
